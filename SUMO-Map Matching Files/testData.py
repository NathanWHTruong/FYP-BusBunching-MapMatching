...
if __name__ == '__main__':
	...
	longlist = []
	bearlist = []
	with open('latlistfile2.txt','r') as filehandle:
		for line in filehandle:
			currentPlace = line[:-1] #get all but /n
			latlist.append(currentPlace)
	with open('longlistfile2.txt','r') as filehandle:
		for line in filehandle:
			currentPlace = line[:-1] #get all but /n
			longlist.append(currentPlace)
	with open('bearlistfile2.txt','r') as filehandle:
		for line in filehandle:
			currentPlace = line[:-1] #get all but /n
	...
	for j in range(len(longlist)): 
		xnet, ynet = net.convertLonLat2XY(float(longlist[j]),float(latlist[j]))
		xnetlist.append(xnet)
		ynetlist.append(ynet)
