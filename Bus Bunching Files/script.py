	...
	#check if person ready to be swapped 
		for personinfo in peopleSpawnInfo[:]: #use a copy so can modify original 	
			...
			if int(busCurrStop[firstBus]) >= int(personinfo[3])-1:	
				travelTime1= calcTimeForBus("b"+str(firstBus),currPersonEdge)
				travelTime2= calcTimeForBus("b"+str(int(firstBus)+1),currPersonEdge)
				#print(travelTime1,travelTime2)
				if travelTime2 == 0:
					#catch the next one guaranteed 
				#	print(personinfo[0]+ 'catch the first one cuz there is no other one')
				#	print(busCurrStop)
					traci.person.removeStages(personinfo[0])
					deletedPeople.append([personinfo[0],traci.person.getEdges(personinfo[0]),personinfo[1]])
					...
					prob = humanModelDecider(travelTime2-travelTime1,busCapacity[firstBus],alpha,Tmax)
					if random.random() < prob or busCapacity[firstBus] >= 70: #wait for next one instead and append to new peopleSpawnInfo
					...
					#	print(personinfo[0]+ 'NOO LET ME CATCH RIGHT NOW!' + str(firstBus))
					#	print(busCurrStop)
